...
func NewRequest[M MyType](method string, target string, body M) *http.Request {
	if method == "" {
		method = "GET"
	}
	req, err := http.ReadRequest(bufio.NewReader(strings.NewReader(method + " " + target + " HTTP/1.1\r\n\r\n")))
	if err != nil {
		panic(err)
	}
...
func main() {
	...
	fmt.Println(NewRequest("GET",
		"/", myStruct{}).ContentLength)
	fmt.Println(NewRequest("GET", "/", strings.NewReader("")).ContentLength)
	fmt.Println(NewRequest("GET", "/",
		&bytes.Buffer{}).ContentLength)
